...
static int mma7660_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	input_set_abs_params(mma7660_dev, ABS_X, -32, 31, 0, 0);
	input_set_abs_params(mma7660_dev, ABS_Y, -32, 31, 0, 0);
	input_set_abs_params(mma7660_dev, ABS_Z, -32, 31, 0, 0);
...
static int mma7660_resume(struct device *dev) {
    printk("*** %s ***\n", __func__);
    mma7660_enable(mma7660_client, 1);
    return 0;
}

static const struct dev_pm_ops mma7660_pm_ops = {
