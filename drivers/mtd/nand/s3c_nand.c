...
{
	struct s3c2410_platform_nand *plat_info = pdev->dev.platform_data;
    ...
    if (plat_info == NULL) {
		dev_err(&pdev->dev, "no platform data defined for s3c_nand");
        return -ENOENT;
    }

    if (plat_info->nr_sets != 1) {
		dev_err(&pdev->dev, "must have only one set for s3c_nand");
        return -ENOENT;
    }

    set = &plat_info->sets[0];
    partition_info = set->partitions;
	...
	for (i = 0; i < set->nr_chips; i++) {
		...
		if (num_partitions <= 0) {
			...
			num_partitions = set->nr_partitions;
