...
static int hm2055_s_config(struct v4l2_subdev *sd, struct midcam_platform_data *pdata) {
    ...
    if (!(pdata->default_width && pdata->default_height)) {
        state->fmt.width = hm2055_framesize_list->width;
        state->fmt.height = hm2055_framesize_list->height;
    } else {
...
static int hm2055_enum_mbus_fmt(struct v4l2_subdev *sd, unsigned int index,
...
static int hm2055_g_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt) {
    struct hm2055_state *state = to_state(sd);
    *fmt = state->fmt;
    return 0;
}

static int hm2055_try_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt) {
    struct i2c_client *client = v4l2_get_subdevdata(sd);
	int num_entries;
	int i;
...
static int hm2055_s_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt) {
    struct hm2055_state *state = to_state(sd);
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    ...
    if (!(fmt->width == 640 && fmt->height == 480) && 
            !(fmt->width == 1600 && fmt->height == 1200)) {
		dev_err(&client->dev,
...
static const struct v4l2_subdev_video_ops hm2055_video_ops = {
    ...
    .enum_mbus_fmt = hm2055_enum_mbus_fmt,
    .g_mbus_fmt = hm2055_g_mbus_fmt,
    .try_mbus_fmt = hm2055_try_mbus_fmt,
    .s_mbus_fmt = hm2055_s_mbus_fmt,
...
static struct i2c_driver v4l2_i2c_driver = {
    .driver.name = HM2055_DRIVER_NAME,
    .probe = hm2055_probe,
...
MODULE_DESCRIPTION("Himax HM2055 UXGA camera driver");
