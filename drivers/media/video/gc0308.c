...
static int gc0308_s_config(struct v4l2_subdev *sd, struct midcam_platform_data *pdata) {
    ...
    if (!(pdata->default_width && pdata->default_height)) {
        state->fmt.width = gc0308_framesize_list->width;
        state->fmt.height = gc0308_framesize_list->height;
    } else {
...
static int gc0308_g_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt) {
    struct gc0308_state *state = to_state(sd);
    *fmt = state->fmt;
    return 0;
}

static int gc0308_try_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt) {
    struct i2c_client *client = v4l2_get_subdevdata(sd);
	int num_entries;
	int i;
...
static int gc0308_s_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt) {
    struct gc0308_state *state = to_state(sd);
    struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static struct i2c_driver v4l2_i2c_driver = {
    .driver.name = GC0308_DRIVER_NAME,
    .probe = gc0308_probe,
...
MODULE_DESCRIPTION("Galaxycore GC0308 VGA camera driver");
